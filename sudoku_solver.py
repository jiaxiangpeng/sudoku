...
class Sudoku(object):
    ...
    def load(self, string):
        values = numpy.reshape([int(c) for c in string], (9,9))
        self.given = Given(values)
        return 

    def save(self, path, solution):
        # Save a configuration to a file.
        print(solution.values.reshape(Nd*Nd))
...
def solver(input):
    ...
    if(solution):
        output = ''
        for num in solution.values.reshape(Nd*Nd):
            output = output + str(int(num))
    else:
        output = '0'*81
    endTime = time.time()
    print('cost time is %ds'%(int(endTime-startTime)))
    return output
...
#     string = data['quizzes'][i]
#     s.load(string=string)
#     solution = s.solve()
#     if(solution):
#         resultNum += 1
#         result = ''
#         for num in solution.values.reshape(Nd*Nd):
#             result = result + str(int(num))
#         if result == data['solutions'][i]:
#             successNum += 1

#         resultList.append(result)
#     else:
#         resultList.append('0'*81)

# print('result rate :', resultNum * 1.0 / len(data))
# print('accuracy :', successNum * 1.0 / len(data))
